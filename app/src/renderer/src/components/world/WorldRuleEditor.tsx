...
export default function WorldRuleEditor({ entityId, onWordCountChange }: WorldRuleEditorProps) {
  const { worldRules, markTabDirty, updateWorldRule, drafts, setDraft, clearDraft, setSelectedText } = useWorkspaceStore()
  ...
  const autosave = useCallback(
    debounce(async (html: string, currentTitle: string) => {
      ...
      try {
        const updated = await window.api.worldRules.update({ id: entityId, content: html, title: currentTitle })
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({ placeholder: 'Write world rule details here...' }),
      CharacterCount,
      SearchAndReplace,
    ],
  ...
  if (!item) return <div>World rule not found</div>
